...
export async function GET(request) {
  try {
    ...
    const provinceFilter = province_id
      ? `WHERE provincial_office = ${province_id}`
      : "";

    const statsQuery = `
      SELECT 
        (SELECT COUNT(*) FROM applications ${provinceFilter}) as total_applications,
        (
          SELECT COUNT(*) 
          FROM applications 
          WHERE ${province_id ? `provincial_office = ${province_id} AND ` : ""}(
    ...
    const monthlyQuery = `
      ...
      ${province_id ? `AND provincial_office = ${province_id}` : ""}
    ...
    const recentQuery = `
      ...
      ${province_id ? `WHERE a.provincial_office = ${province_id}` : ""}
