...
const fetchApplications = async () => {
  const response = await fetch("/api/oiccao/applications");
  if (!response.ok) {
...
const fetchApplicationById = async (id) => {
  try {
    const response = await fetch(`/api/oiccao/applications/${id}`);
    const data = await response.json();
...
export const useApplication = (id) => {
  ...
  const deleteApplicationMutation = useMutation({
    mutationFn: async (applicationId) => {
      const response = await fetch(`/api/oiccao/applications/${applicationId}`, {
        method: "DELETE",
      });
      if (!response.ok) {
